...
then
    ...
    # Set HDF5_NO_HDF5_CMAKE to avoid using problematic HDF5 CMake config files
    # that define targets like 'hdf5-shared' instead of 'hdf5'
    HDF5_INFO="-DHDF5_ROOT:PATH=${INSTALL_PATH} -DHDF5_DIR:PATH=${INSTALL_PATH} -DHDF5_NO_HDF5_CMAKE:BOOL=ON -D${PREFIX}ENABLE_HDF5:BOOL=ON -D${PREFIX}PLUGIN_INSTALL_DIR=YES"
else
