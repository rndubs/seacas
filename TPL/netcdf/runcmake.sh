...
then
    ...
    # Explicitly set HDF5 paths to avoid using problematic HDF5 CMake config files
    # that define targets like 'hdf5-shared' instead of 'hdf5'
    HDF5_INFO="-DHDF5_ROOT:PATH=${INSTALL_PATH} -DHDF5_DIR:PATH=${INSTALL_PATH} -DHDF5_NO_HDF5_CMAKE:BOOL=ON"
    HDF5_INFO="${HDF5_INFO} -DHDF5_INCLUDE_DIR:PATH=${INSTALL_PATH}/include"
    HDF5_INFO="${HDF5_INFO} -DHDF5_C_LIBRARY:FILEPATH=${INSTALL_PATH}/lib/libhdf5.${LD_EXT}"
    HDF5_INFO="${HDF5_INFO} -DHDF5_HL_LIBRARY:FILEPATH=${INSTALL_PATH}/lib/libhdf5_hl.${LD_EXT}"
    HDF5_INFO="${HDF5_INFO} -D${PREFIX}ENABLE_HDF5:BOOL=ON -D${PREFIX}PLUGIN_INSTALL_DIR=YES"
else
