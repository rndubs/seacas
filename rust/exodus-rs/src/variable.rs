...
mod tests {
    ...
    fn test_global_variables() {
        ...
        // Write
        {
            ...
            file.define_variables(EntityType::Global, &["KE", "PE"])
                .unwrap();

            // Write 3 time steps
            for step in 0..3 {
                file.put_time(step, (step as f64) * 0.1).unwrap();
                file.put_var(step, EntityType::Global, 0, 0, &[10.0 * step as f64])
                    .unwrap();
                file.put_var(step, EntityType::Global, 0, 1, &[20.0 * step as f64])
                    .unwrap();
            }
        }
    ...
    fn test_nodal_variables() {
        ...
        // Write
        {
            ...
            file.put_var(1, EntityType::Nodal, 0, 0, &[150.0, 250.0, 350.0, 450.0])
                .unwrap();
        }
        ...
        {
            let file = ExodusFile::<mode::Read>::open(tmp.path()).unwrap();
            let names = file.variable_names(EntityType::Nodal).unwrap();
            assert_eq!(names, vec!["Temperature"]);

            let temp_t0 = file.var(0, EntityType::Nodal, 0, 0).unwrap();
            assert_eq!(temp_t0, vec![100.0, 200.0, 300.0, 400.0]);

            let temp_t1 = file.var(1, EntityType::Nodal, 0, 0).unwrap();
            assert_eq!(temp_t1, vec![150.0, 250.0, 350.0, 450.0]);
        }
    }
    ...
    fn test_element_variables() {
        ...
        {
            let file = ExodusFile::<mode::Read>::open(tmp.path()).unwrap();
            let names = file.variable_names(EntityType::ElemBlock).unwrap();
            assert_eq!(names, vec!["Stress"]);
