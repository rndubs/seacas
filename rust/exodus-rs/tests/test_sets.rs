...
// NOTE: Testing multiple sets in a single file is currently not supported
// due to NetCDF define/data mode transition limitations. Each test creates
// a separate file with a single set. For multiple sets, use the MeshBuilder
// API which handles mode transitions internally.
