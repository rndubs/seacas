...
fn main() -> Result<(), exodus_rs::ExodusError> {
    ...
    println!("\n2. Applying translation (+10, +5, +2)...");
    {
        ...
        for i in 0..4 {
            println!("   Node {}: ({:.2}, {:.2}, {:.2})",
                     i+1, coords.x[i], coords.y[i], coords.z[i]);
        }
    }
    ...
    {
        use exodus_rs::transformations::{rotation_matrix_z, apply_rotation_to_vector};

        let rotation = rotation_matrix_z(PI / 4.0); // 45 degrees in radians
        let point = [1.0, 0.0, 0.0];
        let rotated = apply_rotation_to_vector(&rotation, &point);

        println!("   Original point: ({:.2}, {:.2}, {:.2})", point[0], point[1], point[2]);
        println!("   After 45° Z rotation: ({:.2}, {:.2}, {:.2})",
    ...
    println!("\n7. Tensor transformation example...");
    {
        ...
        println!("   σ_xx={:.2}, σ_yy={:.2}, σ_zz={:.2}",
                 stress_tensor[0], stress_tensor[1], stress_tensor[2]);
        println!("   σ_xy={:.2}, σ_yz={:.2}, σ_xz={:.2}",
